...
class Game {
    constructor(parent, cardSet) {
        ...
        this.onStartTimer.bind(this);
        this.onStartTimer();
        this.onDestroyTimer.bind(this);

        this.gameHeaderRestartButton = this.element.querySelector(".game__header_restart-button");
        ...
        this.gameHeaderRestartButton.addEventListener("click", this.onRestartGameClick.bind(this));
    }
    ...
    onRenderGameInterface() {
        ...
        if (this.cardSet.length === 12) {
            this.element.childNodes[this.element.childNodes.length - 3].classList.add('second-row-centering-if-12-cards');

            const gameHeaderMinPointer = this.element.querySelector(".game__header_min-pointer")
            gameHeaderMinPointer.classList.add('game__header_min-display-adapting-if-12-cards');

            const gameHeaderSinPointer = this.element.querySelector(".game__header_sec-pointer")
            gameHeaderSinPointer.classList.add('game__header_sec-display-adapting-if-12-cards');
        }
        ...
        setTimeout(this.onHideCards.bind(this), 1000);
    }
    ...
    onCheckMatch() {
        ...
        if (this.setPairCard[0].id === this.setPairCard[1].id) {
            ...
            const losePopup = new Popup(this.parent, 'lose', `${this.timerMinutes.textContent}.${this.timerSeconds.textContent}`);
            this.onDestroyTimer()
        }
    ...
    onShowCards(event) {
        ...
        } else if (this.setPairCard.length > 0 && this.setArr.length < this.cardSet.length - 1) {
            this.setArr.push(target);
            this.setPairCard.push(target);
            this.onCheckMatch();
        } else {
            const winPopup = new Popup(this.parent, 'win', `${this.timerMinutes.textContent}.${this.timerSeconds.textContent}`);
            this.onDestroyTimer()
        }
    }
    ...
    onHideCards() {
        for (let i = 1; i <= this.cardSet.length; i++) {
            this.element.childNodes[this.element.childNodes.length - i].style.backgroundImage = "url(./src/img/face.jpg)";
