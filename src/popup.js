class Popup {
    constructor(body, result, stopwatch) {
        ...
        this.popupPlayAgainButton = this.element.querySelector('.popup_play-again-button');
        this.onRestartGameClick.bind(this)
        this.popupPlayAgainButton.addEventListener("click", this.onRestartGameClick.bind(this));
    }
    ...
    renderPopup() {
        if (this.result === 'lose') {
            ...
            this.element = templateEngine(Popup.PopupTemplate(this.result, 'Вы проиграли!', this.stopwatch));
            this.parent.appendChild(this.element);

        } else if (this.result === 'win') {

            this.element = templateEngine(Popup.PopupTemplate(this.result, 'Вы выиграли!', this.stopwatch));
            this.parent.appendChild(this.element);
        }
    }
...
Popup.PopupTemplate = (resultTag, resultText, passedTime) => ({
    ...
    content: [
        ...
        {
            tag: 'button',
            cls: ['popup_play-again-button', `${resultTag}__popup_play-again-button`],
